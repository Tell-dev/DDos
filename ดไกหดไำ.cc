...
int main(int argc, char *argv[])
{
    ...
    // Block bot IPs at the firewall using static routing instead of null routing protocol
    Ptr<Ipv4StaticRouting> firewallRouting = Ipv4RoutingHelper::GetStaticRouting(firewallNode->GetObject<Ipv4>());
    for (int i = 0; i < NUMBER_OF_BOTS; ++i)
    {
        Ptr<Ipv4> botIpv4 = botNodes.Get(i)->GetObject<Ipv4>();
        Ipv4Address botAddress = botIpv4->GetAddress(1, 0).GetLocal();
        firewallRouting->AddHostRouteTo(botAddress, Ipv4Address("0.0.0.0"), 1); // Simulate blocking by not routing bot traffic
    }
