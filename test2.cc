...
int main(int argc, char *argv[])
{
    ...
    extraBulkSend.SetAttribute("MaxBytes", UintegerValue(100));  // กำหนดขนาดแพ็กเก็ตเป็น 100 ไบต์

    // กำหนดอัตราการส่งข้อมูลเป็น 1 Mbps
    DataRate dataRate = DataRate("1Mbps");  // กำหนดอัตราการส่งข้อมูลเป็น 1 Mbps

    // ใช้ OnOffHelper เพื่อกำหนดอัตราการส่งข้อมูลเป็น 1 Mbps
    OnOffHelper onoffTcp("ns3::TcpSocketFactory", Address(InetSocketAddress(i23.GetAddress(1), TCP_SINK_PORT)));
    onoffTcp.SetConstantRate(dataRate);  // กำหนดอัตราการส่งข้อมูลเป็น 1 Mbps

    // ติดตั้งแอปพลิเคชัน OnOffHelper บนโหนด Extra
    ...
    for (int k = 0; k < NUMBER_OF_EXTRA_NODES; k++)
    {
        ...
        // ติดตั้งอัตราการส่งข้อมูล 1 Mbps
        ApplicationContainer onOffAppTcp = onoffTcp.Install(extraNodes.Get(k));
        onOffAppTcp.Start(Seconds(2.0));
        onOffAppTcp.Stop(Seconds(MAX_SIMULATION_TIME));
    }
